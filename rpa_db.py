...
FROM
    ...
    RIGHT JOIN RAYMEDI_CS.med_mrc_hdr mmh ON
        ...
        AND mmh.MMH_NARRATION != 'R260'
        -- AND mmh.MMH_NARRATION = 'R258'
      
        AND MMH_DIST_BILL_DT >= to_date('2025-12-01', 'YYYY-MM-DD')
        AND MMH_DIST_BILL_DT <= to_date('2026-01-05', 'YYYY-MM-DD')
    ORDER BY
...
def process_data_from_db():
    ...
    if not data: 
        ...
        for index, (outlet, invoice) in enumerate(data, start=1):
            try:
                logging.info(f"Processing {index}: Outlet='{outlet}', Invoice='{invoice}'")
                ...
                custom_config = r'--oem 3 --psm 8 -c tessedit_char_whitelist=MR0123456789'
